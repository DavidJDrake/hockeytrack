...
- **`nhl.game.play`** — one event per play. Detail includes `schemaVersion`, `gameId`, `eventId`, `seq`, `playType` (`goal`, `shot-on-goal`, `penalty`, `faceoff`, `hit`, …), team abbreviations, `scoringTeam` (goals only), period/clock, the running score, and the full untouched NHL play object under `raw`.
...
Delivery is at-least-once; dedupe plays on `(gameId, eventId)`. **Not on `seq`**: it is the NHL's sort order, which the NHL renumbers. At the start of each period it appends the period-start and opening faceoff with provisional numbers in the 9000s and gives them their real ones about a minute later; it also inserts plays late, below numbers already published. A consumer that keeps a highest-`seq` mark will, sooner or later, skip real plays. (This project's own poller did exactly that until 2026-09-20: one poll that caught a provisional 9004 stopped every later play of that game from being published. It now tracks what it has sent by `eventId`.) Use `seq` to order plays, never to identify them, and treat a `seq` of 9000 or more as provisional: the play is real and is published at once rather than a minute late, but its place in the order is not settled yet, and it will not be published again when the NHL renumbers it. Also treat `nhl.game.clock` as a snapshot keyed by `(gameId, observedAt)` (apply the newest, drop older ones), and treat status, roster and final events as idempotent. Every event carries `schemaVersion` so the schema can evolve without breaking you.
